...
(function (countapi, yaml, fs) {
  ...
  function loadNbVisitors() {
    console.log("Oui");
    countapi.visits().then(function (result) {
      var changeConfig = function changeConfig(counter) {
        doc.counters[0].counter = {};
        delete doc.counters[0].counter;
        doc.counters[0].counter = result.value;
        fs.writeFileSync('/_data/visitors.yml', yaml.dump(doc));
      };
      var doc = yaml.load(fs.readFileSync('/_data/visitors.yml', 'utf-8'));
      changeConfig();
    });
  }
